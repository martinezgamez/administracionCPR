...
<head>
    <meta charset="UTF-8">
    <title>recibo de locomoción</title>
...
<body>
    <div class="content">
        <h1 class="text-center informe-titulo">Recibo de locomoción</h1>

        <p><b>D./Dña. </b> {{ $ponente->NOMBRE }} {{ $ponente->APELLIDO1 }} {{ $ponente->APELLIDO2 }}</p>

        <p><b>DNI: </b>{{ $ponente->DNI }}</p>

        <p>Ha recibido de la Dirección Provincial del MECD de Melilla la cantidad de: <span>{{ $gastosLetras }}</span>, ({{ $gastos }}€), correspondientes a los gastos de locomoción por la realización de la actividad formativa: "{{ $curso->NOMBRE }}", celebrada en Melilla los días {{ $curso->INICIO }} al {{ $curso->FIN }}</p>
