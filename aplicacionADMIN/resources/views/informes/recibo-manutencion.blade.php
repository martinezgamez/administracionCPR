...
<head>
    <meta charset="UTF-8">
    <title>recibo de manutención</title>
...
<body>
    <div class="content">
        <h1 class="text-center informe-titulo">Recibo de manutención</h1>

        <p><b>D./Dña. </b> {{ $ponente->NOMBRE }} {{ $ponente->APELLIDO1 }} {{ $ponente->APELLIDO2 }}</p>

        <p><b>DNI: </b>{{ $ponente->DNI }}</p>

        <p>Ha recibido de la Dirección Provincial del MECD de Melilla la cantidad de: <span>{{ $gastosLetras }}</span>, ({{ $gastos }}€), en concepto de compensación por los gastos ocasionados (comida y/o cena) con motivo de la realización de la actividad formativa: "{{ $curso->NOMBRE }}", celebrada en Melilla los días {{ $curso->INICIO }} al {{ $curso->FIN }}</p>
